...
class Classifier:
    ...
    def classify(self):
        ...
        # Read features
        features_names = ('swear_word_contained', 'offensiveness_score', 'urls_count', 
                        'user_mentions_count', 'letters_count', 'upper_letters_sum', 
                        'ratio_letters_upperletters', 'polarity', 'subjectivity', 
                        'retweets_count', 'favourite_count', 'friends_count', 'followers_count', 
                        'ratio_followers_friends', 'total_posts_count', 'account_age')
        labels_names = ['abusive', 'hateful', 'normal']
        # Drop unnecessary features (columns 2, 4, 6 and 19) from the dataset
        features = pd.read_csv(file_in, names=features_names, usecols = [1,3,5,20] + range(7,19))
        ...
        for train_index, test_index in kf.split(features):  
            ...
            precision_classes.append(precision_score(labels_test, prediction, average=None))
            precision_total.append(precision_score(labels_test, prediction, average='weighted'))
            recall_classes.append(recall_score(labels_test, prediction, average=None))
            recall_total.append(recall_score(labels_test, prediction, average='weighted'))
            fscore_classes.append(f1_score(labels_test, prediction, average=None))
            fscore_total.append(f1_score(labels_test, prediction, average='weighted'))
        ...
        precision = np.mean(precision_classes, axis=0).tolist() + [np.mean(precision_total,axis=0)]
        recall = np.mean(recall_classes, axis=0).tolist() + [np.mean(recall_total,axis=0)]
        fscore = np.mean(fscore_classes, axis=0).tolist() + [np.mean(fscore_total,axis=0)]
        report_array = np.asarray([precision, recall, fscore])
        report = pd.DataFrame(report_array, ['precision', 'recall', 'f1-score'], [labels_names + ['avg/total']])
        print report

        # Feature importances average
        importances_average = np.mean(importances_sum, axis=0).tolist()
        feature_importances = [(feature, round(importance, 4)) for feature, importance in zip(features_names, importances_average)]
        ...
        export_graphviz(one_tree, out_file='one_tree.dot', max_depth=5, feature_names=features_names, class_names=labels_names, rounded = True, precision=1)
