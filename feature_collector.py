...
"""
FeatureCollector connects to Twitter API and collects features from tweets 
given their IDs
"""
class FeatureCollector:
    def __init__(self, file_preprocessed_path):
        ...
        file_preprocessed_path, file_preprocessed_extension = 
            os.path.splitext(self.file_preprocessed_path)
        self.file_out_path = file_preprocessed_path + '-features_collected' 
            + file_preprocessed_extension
    ...
    def get_features(self, tweet_id):
        ...
        try:
            ...
            upper_letters_sum = sum(1 for letter in tweet_text 
                if letter.isupper())
            try:
                ratio_letters_upperletters = round(letters_count/
                    upper_letters_sum, 2)
            ...
            hatebase_data = open('_data\hatebase_data.csv', 'r')
            ...
            swearword_data = xlrd.open_workbook('_data\swearword_data.xlsx')
            swear_word_contained = 0
            for row in range(swearword_data.sheet_by_index(0).nrows):
                if swearword_data.sheet_by_index(0).cell_value(row, 0) 
                    in tweet_text:
